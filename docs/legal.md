...
Nothing has ever been collected. `.github/FUNDING.yml` ships entirely commented
out, every `SUPPORT_PLATFORMS` entry is `live: false`, and there is no `Funding`
URL in `pipeline/pyproject.toml` — `verify_support.mjs` reads all three and
fails if they disagree, so a channel cannot be advertised in one place while the
others call it closed.
...
the recipient does. Both matter on the day a channel opens, and none of it is
live while every platform is `live: false`.
