...
| **P2** | **A shared number must not reconstruct a private one.** `extraPerMonth = salary × π/(100+π)` inverts exactly, so a € absolute beside the percentage it came from reveals the salary. Share surfaces carry percentages, ranks and category names. Check every new card against the inversion, not against intent. |
