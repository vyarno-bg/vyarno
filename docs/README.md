...
| Add a new data source **to the site** | [`data-sources.md`](./data-sources.md) → [`math.md`](./math.md) → the pattern in `sources/eurostat.py` |
...
| Change a response header | `site/public/_headers` → `verify_static_assets.mjs`. That file is the declaration; applying it is the deployment's job, and this repository does not describe one |
...
- **No upstream notifications.** Nothing tells us a publisher released a new
  ...
  `view.js#dataAge`: each of the eight is judged against the cadence its
  manifest row declares (`site/src/lib/payloads.js`), and the banner fires when
  any is overdue. Which also means automating seven of the eight would not clear
  it — the panel would still show the eighth going red on its own.
