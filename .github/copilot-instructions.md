...
Run `make check` before calling a change done. It prints each suite's count and
fails if one has shrunk, so there is no number to check by eye — and never write
one into a doc. Run bare, the render suite skips and exits 0 where it finds no
...
- A comment states the constraint, never the diff. No comment may describe an
  ...
  **Nothing enforces this; it is on you and on review**, because a regex over
  prose cannot tell that phrasing from a legitimate description of an upstream
  that changed. You will reach for it while editing, because the delta is what
  is freshest in your head. Ask what the old version got wrong and write that as
  a rule the code has to keep.
...
- No em-dashes in user-facing copy. A full stop, a colon or a comma instead.
