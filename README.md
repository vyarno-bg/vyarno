...
the site does is what it does, for everybody. It will be sustained by
donations — there are no salaries, no company and no investor. A donation will
buy nothing: no features, no priority, no influence over any figure the site
publishes. If you would rather not give, nothing changes for you.

**The donation channels are not open yet.** They are declared in
[`.github/FUNDING.yml`](./.github/FUNDING.yml) and
[`site/src/lib/support.js`](./site/src/lib/support.js), and both files stay
commented out until an account actually exists — a donate button pointing at a
404 is worse than no button. `site/scripts/verify_support.mjs` fails the build
if the two ever disagree.

Until then, the most useful support is not money: report a wrong number, fix
