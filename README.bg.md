...
| **НСИ** | средната работна заплата за София — нивото, по което е изравнена подредбата на заплатите — и средната работна заплата по икономически дейности, 19 раздела по КИД-2008, с имената на самите НСИ |
...
flowchart LR
  ...
  J["data/published/*.json<br/>9 JSON файла, в хранилището"]
...
| `pipeline/` | Python 3.11. Сваля данните от Евростат / БНБ / ЕЦБ / имот.bg / НСИ, плюс датираните таблици на осигурителното законодателство и кредитните ограничения, зад проверките. CLI: `vyarno-pipeline refresh --source <name>`. Пише девет JSON файла в `data/published/` |
...
Това записва деветте JSON файла и не комитва нищо — прегледът е самият diff, а
...
Заглавната лента отваря списък с всичките девет — периодът, за който се отнася
