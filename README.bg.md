...
- **Нищо не се крие зад плащане.** Няма регистрация и няма платена версия:
  каквото сайтът показва, го показва на всички.
...
flowchart LR
  ...
  subgraph pipe["pipeline/ · Python, стартиран от наблюдателя"]
    C["конектори"] --> T["преобразуване"] --> G{"проверките"}
  end
...
малък. Версиите са в `pipeline/pyproject.toml` и `site/package.json`; таблица тук
би била второ копие, което само се разминава с тях. Python 3.11 и Node 22, и
двете фиксирани в CI.
...
|---|---|---|
| `pytest` в `pipeline/` | без интернет | Конекторите, преобразуванията, проверките, публикуваните файлове |
...
| `pipeline/` | Python 3.11. Сваля данните от Евростат / БНБ / ЕЦБ / имот.bg / НСИ, плюс датираните таблици на осигурителното законодателство и кредитните ограничения, зад проверките. CLI: `vyarno-pipeline refresh --source <name>`. Пише тринадесет JSON файла в `data/published/` |
...
| `site/` | Vite 8 + Svelte 5. Шест страници на двата езика — калкулаторът, `/how/`, `/market/`, `/credit/`, `/legal/`, `/support/` — плюс 404. Сглобява се в статична директория |
...
Вярно е две неща: статичен билд и конвейер. `npm run build` произвежда
`site/dist/` — директория с файлове без нищо сървърно; всеки статичен хостинг ще
се справи с нея. `site/public/_headers` е политиката за сигурност и кеширане,
която внедряването трябва да приложи, независимо дали хостингът чете този формат
директно, или ти го превеждаш на синтаксиса на своя сървър.
...
Това записва тринадесетте JSON файла и не комитва нищо — прегледът е самият
diff, а файл, който никой не е погледнал, е число, което никой не е проверил.
Всеки `--source` може да се пусне сам; `vyarno-pipeline refresh --help` ги
изброява. Тук ги стартира наблюдател на изданията, а не човек:
`.github/workflows/watch.yml` проверява всеки източник в прозореца, който
[`release_calendar.py`](./pipeline/src/vyarno_pipeline/release_calendar.py)
записва за него, и отваря pull request до десетина минути след публикуването.
**Нищо не слива този pull request автоматично** — това е единственият път, по
който число стига до читател.

**Как да разбереш, че обновяването е закъсняло.** Всеки файл носи ритъма на
източника, от който идва, и се мери спрямо своя собствен — месечен за ХИПЦ,
тримесечен за заплатите на НСИ, веднъж на четири години за изследването на
заплатите на Евростат
([`site/src/lib/payloads.js`](./site/src/lib/payloads.js)). След срока си файлът
е «предстои обновяване», а след 1,5 пъти срока е «закъснял» и сайтът показва
лента. Един общ праг не може да обслужи тези ритми: 45 дни са закъснение за
месечна серия, нормални за тримесечна и без значение за изследване веднъж на
четири години.

Заглавната лента отваря списък с всяко число — периодът, за който се отнася,
денят, в който сме го изтеглили, и връзка към първоизточника. Двете дати са
различни (числата за юни, изтеглени на 27 юли), и точно тук престават да се
сливат в една.
...
единственото копие, което има значение, е това, което работи на vyarno.bg.
Вместо това качената страница носи `/version.json` по-горе, записан от билда, та
не може да остарее. Ако някога нещо зависи от конкретно състояние на това
хранилище, отговорът се променя с него.

## Как да помогнеш

Всяко участие е добре дошло, а **поправките по числата — най-много от всичко**.
Ако някое число изглежда сгрешено, това е сигналът с най-голяма стойност, който
можеш да подадеш: гражданско средство, което греши, е по-лошо от никакво. Не
трябва да си програмист, за да съобщиш за сгрешено число, за умрял `source_url`
или да поправиш тромав български.

[`CONTRIBUTING.md`](./CONTRIBUTING.md) описва подготовката, начините да помогнеш
по полза и едно твърдо правило за данните: нов източник идва заедно с условията
си за ползване, цитирани дословно, на оригиналния език и с датата, на която са
прочетени. [`CODE_OF_CONDUCT.md`](./CODE_OF_CONDUCT.md) важи за всяко
пространство, което проектът използва.
...
Всичко в сайта е безплатно за всички, няма акаунт и няма платена версия.
Издържа се от дарения, а **дарението не купува нищо** — нито функции, нито
приоритет, нито влияние върху което и да е публикувано число. Това последното
не се променя, дори ако някой ден проектът продава нещо: дарението си остава
дарение. Сайтът казва същото и сам, на
