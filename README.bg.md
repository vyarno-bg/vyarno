...
pytest -q                                     # 288 теста, без интернет
...
`make check` от корена на хранилището (`make help` изброява целите).
...
Седем въпроса, от една нетна заплата и една кошница:
...
| **Къде съм по заплата?** | Подредба на брутните заплати в София в 11 стъпки, от P1 до P99 — формата на разпределението е от изследване на Евростат, а нивото е изравнено по последната средна заплата на НСИ |
...
| **Колко далеч е един дом?** | Анюитет по лихвата на ЕЦБ за нови кредити, в рамките на ограниченията на БНБ (LTV, DSTI, максимален срок), срещу цените €/м² в 143 квартала на София — с праг на достъпност, който съзнателно държим строг: 30% от нетния доход |
...
| Източник | Какво дава |
|---|---|
| **Евростат** | ХИПЦ — инфлацията по 13 групи и ~46 подгрупи, теглата на официалната кошница, индексите по години и формата на разпределението на заплатите |
| **ЕЦБ / БНБ** | лихвите по нови жилищни кредити, ГПР, ограниченията на БНБ за ипотеки (LTV, DSTI, максимален срок) |
| **НСИ** | средната работна заплата за София и нивото, по което е изравнена подредбата на заплатите |
| **имот.bg** | средни цени €/м² по квартали в София |
...
flowchart LR
  ...
  subgraph pipe["pipeline/ · Python, пуска се ръчно"]
    C["конектори"] --> T["преобразуване"] --> G{"6 проверки"}
  end

  J["data/published/*.json<br/>8 файла, в хранилището"]
...
- **Страницата не зарежда нищо чуждо.** Няма скрипт от CDN, няма шрифт от чужд
  сървър, няма пиксел за анализ. Шрифтовете се зареждат от нашия домейн —
  собствените файлове на IBM и Adobe, вградени без никаква промяна и никога
  орязвани тук, защото и двете семейства носят запазено име, а OFL 1.1 смята
  орязването до подмножество за промяна (`NOTICE`). Политиката CSP в
  `site/public/_headers` е това, което прави твърдението проверимо, а не просто
  заявено.
...
|---|---|---|
| `pytest` в `pipeline/` | 288 теста, без интернет | Конекторите, преобразуванията, шестте проверки, публикуваните файлове |
| `node:test` в `site/` | 315 теста | Всяка формула, всяка производна стойност, инвариантите на текста, правните твърдения, контраста по WCAG, HTTP заглавките |
| `node:test` + Playwright | 15 теста | Готовата страница, заредена в истински браузър — единственият набор, който пуска самото приложение |

`make check` пуска всичко в реда на CI. Числата по-горе са записани на ръка, а не
отчетени от значка: те са това, което наборите отпечатват, а брой, който се е
променил, без някой да го е местил, е находка, не шум.
...
| `pipeline/` | Python 3.11. Сваля данните от Евростат / БНБ / ЕЦБ / имот.bg / НСИ, плюс датираните таблици на осигурителното законодателство и кредитните ограничения, зад проверките. CLI: `vyarno-pipeline refresh --source <name>`. Пише осем JSON файла в `data/published/` |
...
| `.github/workflows/ci.yml` | Двата набора тестове и продукционният билд, при всеки push към всеки клон. Не обновява данните |
...
Това записва осемте JSON файла и не комитва нищо — прегледът е самият diff, а
...
Заглавната лента отваря списък с всичките осем — периодът, за който се отнася
...
**[Ko-fi](https://ko-fi.com/vyarno)** — еднократно, без регистрация. Това е
единственият отворен канал. Останалите три са обявени в
[`.github/FUNDING.yml`](./.github/FUNDING.yml) и
[`site/src/lib/support.js`](./site/src/lib/support.js) и стоят изключени,
докато реално не съществува сметка — бутон за дарение, който води към 404, е
по-лош от никакъв бутон. `site/scripts/verify_support.mjs` чупи билда, ако двата
файла някога се разминат.
